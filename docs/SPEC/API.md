...
プライバシー保護とセキュリティ強化のため、以下の方針を採用しています：

- **リソースアクセス権限エラー**: `404 Not Found`を返却
  - 他のユーザーのタスクリスト・タスクへのアクセス試行
  - 存在しないリソースへのアクセス試行
  - 理由: リソースの存在情報を隠蔽し、情報漏洩を防止

- **認証エラー**: `401 Unauthorized`を返却
  - 無効なJWTトークン、期限切れトークン

- **バリデーションエラー**: `400 Bad Request`を返却
  - 不正なリクエストフォーマット、必須フィールド不足

この方針により、攻撃者がリソースの存在を推測することを困難にし、ユーザーのプライバシーを保護します。
